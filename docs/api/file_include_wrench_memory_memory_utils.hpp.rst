
.. _file_include_wrench_memory_memory_utils.hpp:

File memory_utils.hpp
=====================


This file defines utility functions for memory related operations. 



.. contents:: Contents
   :local:
   :backlinks: none

Definition (``include/wrench/memory/memory_utils.hpp``)
-------------------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_include_wrench_memory_memory_utils.hpp.rst





Includes
--------


- ``cassert``

- ``cstdint``



Included By
-----------


- :ref:`file_include_wrench_memory_arena.hpp`

- :ref:`file_include_wrench_memory_linear_allocator.hpp`

- :ref:`file_include_wrench_memory_pool_allocator.hpp`




Namespaces
----------


- :ref:`namespace_wrench`


Functions
---------


- :ref:`exhale_function_memory__utils_8hpp_1a5895da501ef40348ce77c4dde12e3179`

- :ref:`exhale_function_memory__utils_8hpp_1ab4a31f5ef79d511a861dcf2c23c5ef55`

