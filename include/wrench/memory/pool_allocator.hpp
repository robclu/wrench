...
/// This type is a simple, single-threaded free-list implementation. It is
...
class ThreadSafeFreelist {
  ...
  /// therefore moves the head. The other thread then does a push_front, and
 ...
 private:
  AtomicHeadPtr head_{};            //!< Head pointer (index).
  Node*         storage_ = nullptr; //!< Storage.
};
...
/// Allocator to allocate elements of size ElementSize, with a given Alignment,
...
class PoolAllocator {
 ...
 private:
  ...
  const void*  begin_ = nullptr; //!< The beginning of the pool arena.
  const void*  end_   = nullptr; //!< The end of the pool arena.
};
